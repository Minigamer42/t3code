...
export const ComposerPrimaryActions = memo(function ComposerPrimaryActions({
  ...
  const renderStopGenerationButton = (insidePendingAction: boolean) => (
    <button
      type="button"
      className={cn(
        "flex cursor-pointer items-center justify-center rounded-full bg-destructive/90 text-white shadow-xs shadow-destructive/24 inset-shadow-[0_1px_--theme(--color-white/16%)] transition-all duration-150 hover:bg-destructive hover:scale-105 active:inset-shadow-[0_1px_--theme(--color-black/8%)] active:shadow-none disabled:cursor-wait disabled:hover:scale-100",
        insidePendingAction
          ? "size-8 sm:size-7"
      ...
      onClick={onInterrupt}
      aria-label={isInterruptPending ? "Stop requested" : "Stop generation"}
      title={isInterruptPending ? "Stop requested — waiting for the provider" : undefined}
    >
      {isInterruptPending ? (
