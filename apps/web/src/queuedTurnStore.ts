...
function nextThreadState(state: QueuedTurnStoreState, threadKey: string): QueuedTurnThreadState {
...
export const useQueuedTurnStore = create<QueuedTurnStoreState>()((set) => ({
  ...
  enqueue: (threadKey, submission) => {
    set((state) => {
      const current = nextThreadState(state, threadKey);
      return {
        byThreadKey: {
          ...state.byThreadKey,
          [threadKey]: {
...
export function queuedTurnSubmissionToChatMessage(submission: QueuedTurnSubmission): ChatMessage {
  const createdAt = submission.input.createdAt ?? submission.settings.createdAt;
  const attachments = submission.input.message.attachments.map((attachment, index) => ({
    type: attachment.type,
    id: submission.composer.images[index]?.id ?? `${submission.messageId}-${index}`,
