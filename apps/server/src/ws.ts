...
const RPC_REQUIRED_SCOPE = new Map<string, AuthEnvironmentScope>([
  ...
  [WS_METHODS.gitListRewriteableCommits, AuthOrchestrationReadScope],
  [WS_METHODS.gitRewriteCommitMessages, AuthOrchestrationOperateScope],
  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      return WsRpcGroup.of({
        ...
        [WS_METHODS.gitListRewriteableCommits]: (input) =>
          observeRpcEffect(
            WS_METHODS.gitListRewriteableCommits,
            gitWorkflow.listRewriteableCommits(input),
            { "rpc.aggregate": "git" },
          ),
        [WS_METHODS.gitRewriteCommitMessages]: (input) =>
          observeRpcEffect(
            WS_METHODS.gitRewriteCommitMessages,
            gitWorkflow
              .rewriteCommitMessages(input)
              .pipe(Effect.tap(() => refreshGitStatus(input.cwd))),
            { "rpc.aggregate": "git" },
          ),
