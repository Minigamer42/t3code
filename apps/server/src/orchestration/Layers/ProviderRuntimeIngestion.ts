...
export function runtimeEventToActivities(
  ...
  switch (event.type) {
    ...
    case "item.completed": {
      ...
      const eventData = Predicate.isObject(event.payload.data) ? event.payload.data : undefined;
      const existingRawOutput = Predicate.isObject(eventData?.rawOutput)
        ? eventData.rawOutput
        : undefined;
...
const make = Effect.gen(function* () {
  ...
  const processRuntimeEvent = (event: ProviderRuntimeEvent) =>
    Effect.gen(function* () {
      ...
      const activities = runtimeEventToActivities(activityEvent, taskTitle, completedToolOutput);
